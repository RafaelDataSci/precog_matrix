...
onehot_columns = ['AUTO THEFT', 'ASSAULT', 'ROBBERY', 'THEFT OVER', 'BREAK AND ENTER', 'HOMICIDE']  # Tem que ajustar, pq não sei os nomes
...
def predict(df_processed, model):
   ...
   # Make predictions (predict_proba returns the probability of each class)
    probabilities = (model.predict_proba(df_processed)) * 100
